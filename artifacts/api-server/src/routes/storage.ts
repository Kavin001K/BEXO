...
async function callGemini(
  ...
  if (key) {
    ...
    try {
      ...
      console.warn(`[Gemini] Direct SDK failed: ${err.message}`);
      // If key is leaked (403), we definitely want to try OpenRouter
  ...
  if (openRouterKey) {
    console.info(`[Gemini] Attempting OpenRouter fallback for ${modelToUse}...`);
    try {
      const orModel = modelToUse.startsWith("google/") ? modelToUse : `google/${modelToUse}`;
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: Array.isArray(prompt) 
            ? prompt.map(p => ({ role: "user", content: typeof p === "string" ? p : JSON.stringify(p) }))
      ...
      console.error(`[Gemini] OpenRouter failed: ${err.message}`);
  ...
  if (!modelNameOverride && modelToUse !== fallbackModel) {
    console.warn(`[Gemini] Retrying with fallback model ${fallbackModel}...`);
    return callGemini(prompt, fallbackModel);
  }

  throw new Error("AI parsing failed: No valid API keys or models available.");
}
